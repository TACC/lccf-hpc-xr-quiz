...
namespace zSpace.zView
{
    ...
    public class ZViewEditor : Editor 
    {
        ...
        SerializedProperty ColorRenderTextureFormatProperty;
        SerializedProperty EnableUsingZCameraCameraAsURPColorCameraProperty;
        SerializedProperty URPColorCameraProperty;
        ...
        SerializedProperty ARModeEnableURPRendererAutoFindProperty;
        SerializedProperty ARModeURPRendererIndexProperty;
        ...
        public override void OnInspectorGUI()
        {
            ...
            this.DrawCommonSettingsSection();
        ...
        private void DrawCommonSettingsSection()
        {
            ZView zView = (ZView)this.target;

            _isCommonSettingsSectionExpanded = this.DrawSectionHeader("Common Settings", _commonSettingsIconTexture, _isCommonSettingsSectionExpanded);
            if (_isCommonSettingsSectionExpanded)
            {
                ...
                EditorGUILayout.PropertyField(
                    this.ColorRenderTextureFormatProperty,
                    new GUIContent(
                        "Color Texture Format",
                        "The texture format to use for color textures. Must " +
                            "be a color texture format that supports the R, " +
                            "G, B, and A channels. Setting this to Default " +
                            "will use the same color texture format that is " +
                            "used internally by zView."));
                EditorGUILayout.Space();

                this.DrawToggleLeft(
                    "Use ZCamera As URP Color Camera",
                    this.EnableUsingZCameraCameraAsURPColorCameraProperty,
                    tooltip:
                        "Enables using the ZCamera camera to render zView " +
                            "color images when the Universal Render " +
                            "Pipeline (URP) is being used.");

                if (!this.EnableUsingZCameraCameraAsURPColorCameraProperty.boolValue)
                {
                    EditorGUILayout.PropertyField(
                        this.URPColorCameraProperty,
                        new GUIContent(
                            "URP Color Camera",
                            "The camera to use for color renders when the " +
                                "Universal Render Pipeline (URP) is being " +
                                "used."));

                    if (this.URPColorCameraProperty.objectReferenceValue == null)
                    {
                        EditorGUILayout.HelpBox(
                            "A Universal Render Pipeline (URP) color camera " +
                                "must be explicitly specified if the " +
                                "ZCamera is not being used as the URP color " +
                                "camera.",
                            MessageType.Error);
                    }
                }
        ...
        private void DrawARModeSection()
        {
            ...
            if (_isARModeSectionExpanded)
            {
                ...
                this.DrawToggleLeft(
                    "Enable URP Renderer Auto-Find",
                    this.ARModeEnableURPRendererAutoFindProperty,
                    tooltip:
                        "Enables logic that attempts to automatically find " +
                            "the zView augmented reality mode Universal " +
                            "Render Pipeline (URP) render when URP is " +
                            "being used.");

                if (!this.ARModeEnableURPRendererAutoFindProperty.boolValue)
                {
                    this.DrawIntField(
                        "URP Renderer Index",
                        this.ARModeURPRendererIndexProperty,
                        tooltip:
                            "The index of the zView augment reality mode " +
                                "Universal Render Pipeline (URP) renderer " +
                                "within the array of renderers for the " +
                                "active Universal Render Pipeline asset. " +
                                "Only needed if URP is being used.");

                    if (this.ARModeURPRendererIndexProperty.intValue < 0)
                    {
                        EditorGUILayout.Space();
                        EditorGUILayout.HelpBox(
                            "When Universal Render Pipeline (URP) renderer " +
                                "auto-finding is disabled and URP is being " +
                                "used, the index of the zView augmented " +
                                "reality mode URP renderer within the array " +
                                "of renderers for the active Universal " +
                                "Render Pipeline asset must be specified " +
        ...
        private void LoadIconTextures()
        {
            ...
            if (_commonSettingsIconTexture == null)
            {
                _commonSettingsIconTexture = this.LoadIconTexture("CommonSettingsIcon.png");
            }
        ...
        private void FindSerializedProperties()
        {
            ...
            this.ColorRenderTextureFormatProperty = this.serializedObject.FindProperty("ColorRenderTextureFormat");
            this.EnableUsingZCameraCameraAsURPColorCameraProperty = this.serializedObject.FindProperty("EnableUsingZCameraCameraAsURPColorCamera");
            this.URPColorCameraProperty = this.serializedObject.FindProperty("URPColorCamera");
            ...
            this.ARModeEnableURPRendererAutoFindProperty = this.serializedObject.FindProperty("ARModeEnableURPRendererAutoFind");
            this.ARModeURPRendererIndexProperty = this.serializedObject.FindProperty("ARModeURPRendererIndex");
        }
        ...
        private void DrawToggleRight(string label, SerializedProperty property, string tooltip = null)
        {
            ...
            property.boolValue = EditorGUILayout.Toggle(guiContent, property.boolValue);
        }

        private void DrawToggleLeft(string label, SerializedProperty property, string tooltip = null)
        {
            string text = " " + label;
            ...
            property.boolValue = EditorGUILayout.ToggleLeft(guiContent, property.boolValue);
        }

        private void DrawLayerMask(string label, SerializedProperty property, string tooltip = null)
        {
            GUIContent guiContent;
            ...
            property.intValue = EditorGUILayout.MaskField(guiContent, property.intValue, _layerNames);
        }

        private void DrawLayerField(string label, SerializedProperty property, string tooltip = null)
        {
            GUIContent guiContent;
            ...
            property.intValue = EditorGUILayout.LayerField(guiContent, property.intValue);
        }

        private void DrawIntField(string label, SerializedProperty property, string tooltip = null)
        {
            GUIContent guiContent;
            ...
            property.intValue = EditorGUILayout.IntField(guiContent, property.intValue);
        }

        private void DrawVector3Field(string label, SerializedProperty property, string tooltip = null)
        {
            GUIContent guiContent;
            ...
            property.vector3Value = EditorGUILayout.Vector3Field(guiContent, property.vector3Value);
        }
        ...
        private Texture2D _commonSettingsIconTexture = null;
        ...
        private bool _isCommonSettingsSectionExpanded = true;
